...
export const FloatingChat: React.FC<FloatingChatProps> = ({ analysisContext }) => {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-96 max-w-[90vw] h-[500px] max-h-[80vh] bg-white rounded-2xl shadow-2xl flex flex-col z-50 border border-gray-200">
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.map((msg, idx) => (
              <div
                key={idx}
                className={`rounded-lg p-3 text-sm ${msg.role === 'user' ? 'bg-gray-100 ml-8' : 'bg-blue-50 mr-8'
                  }`}
              >
